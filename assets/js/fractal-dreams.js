...
(function () {
  ...
  function resize() {
    ...
    canvas.width = (fullscreen ? window.innerWidth : canvas.clientWidth) * dpr;
    canvas.height = (fullscreen ? window.innerHeight : canvas.clientHeight) * dpr;
  ...
  const fsSrc = [
    ...
    '  for(float i=0.0; i<256.0; i++){ z = vec2(z.x*z.x - z.y*z.y, 2.0*z.x*z.y) + c; if(dot(z,z) > 4.0) break; iter++; }',
    '  vec3 col = vec3(0.02, 0.01, 0.04);',
    '  if(iter < 256.0){',
    '    float sl = iter - log2(log2(dot(z,z))) + 4.0;',
    '    float phase = fract(sl * 0.012 + t * 0.008 + audioPhase * 0.1);',
    '    vec3 pal = (phase < 0.33) ? mix(colA, colB, phase/0.33) : (phase < 0.66 ? mix(colB, colC, (phase-0.33)/0.33) : mix(colC, colA, (phase-0.66)/0.34));',
    '    float pulse = 0.05 * sin(audioPhase * 6.283);',
    '    float val = 0.15 + 0.85 * pow(sl/256.0, 0.45) + pulse;',
    '    col = pal * clamp(val + 0.06 * sin(sl*0.3 + t*0.1), 0.0, 1.0);',
    '    col += colA * (1.0 - smoothstep(0.0, 0.3, sl/256.0)) * (0.25 + pulse);',
    '  }',
    '  vec2 uv2 = gl_FragCoord.xy/res; float vig = 1.0 - length((uv2 - 0.5) * 1.2);',
    '  gl_FragColor = vec4(col * (smoothstep(0.0, 0.6, vig) * (0.97 + 0.03 * sin(t*0.15))), 1.0);',
    '}'
  ].join('\n');
  ...
  if (db) db.onclick = () => { driftEnabled = !driftEnabled; db.classList.toggle('is-on', driftEnabled); };

  canvas.onwheel = e => { e.preventDefault(); targetZoom = Math.max(1e-15, Math.min(1e15, targetZoom * (e.deltaY > 0 ? 1.1 : 0.9))); };
  ...
  function frame(ts) {
    ...
    if (driftEnabled) {
      ...
      targetPanX += 0.02 * dt * Math.sin(ts * 0.0002);
      targetPanY += 0.02 * dt * Math.cos(ts * 0.0003);
    }
    ...
    gl.uniform2f(uCenterHi, cxHi, cyHi); gl.uniform2f(uCenterLo, cxLo, cyLo); gl.uniform1f(uPixelScale, pxScale);
